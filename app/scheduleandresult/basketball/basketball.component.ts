...
@Component({
    ...
    templateUrl:'./app/scheduleandresult/basketball/basketball.html',
    styleUrls:['./app/assets/css/scheduleandresults.css', './app/assets/css/common.css']
})

export class BasketBallComponent implements OnInit, AfterViewInit{
    ...
    ngAfterViewInit(){
        jQuery(document).ready(function () {
            ...
            jQuery('#schedule').click(function () {
                ...
                jQuery('.display-space iframe').attr('src','http://indiaatsports.com/widget_matches?tournamentid=5604399795666944');
            ...
            jQuery('#pointstable').click(function () {
                jQuery('.display-space iframe').addClass('hide');
                jQuery('.display-space nav').children().removeClass('active');
                jQuery('.display-space iframe').attr('src','http://indiaatsports.com/widget_pointstable?tournamentid=5604399795666944');
            ...
            jQuery('#participants').click(function () {
                jQuery('.display-space iframe').addClass('hide');
                jQuery('.display-space nav').children().removeClass('active');
                jQuery('.display-space iframe').attr('src','http://indiaatsports.com/displayteamsandplayerslist?tournamentid=5604399795666944&orderingformat=byplayers');
