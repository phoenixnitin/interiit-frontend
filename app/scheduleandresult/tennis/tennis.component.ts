...
@Component({
    selector: 'app-tennis',
    templateUrl:'./app/scheduleandresult/tennis/tennis.html',
    styleUrls:['./app/assets/css/scheduleandresults.css', './app/assets/css/common.css']
})

export class TennisComponent implements OnInit, AfterViewInit{
    ...
    ngAfterViewInit(){
        jQuery(document).ready(function () {
            ...
            jQuery('#participants').click(function () {
                jQuery('.display-space iframe').addClass('hide');
                jQuery('.display-space nav').children().removeClass('active');
                jQuery('.display-space iframe').attr('src','http://indiaatsports.com/displayteamsandplayerslist?tournamentid=4616175233269760&orderingformat=byplayers');
